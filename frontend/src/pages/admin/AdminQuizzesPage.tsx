...
const AdminQuizzesPage: React.FC = () => {
  ...
  const openAssign = async (quiz: any) => {
    ...
    try {
      ...
      if (typeof assignment?.timeLimit === "number") setTimeLimit(assignment.timeLimit);
  ...
  return (
    <div className="space-y-6">
            ...
            <Card key={q._id}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-slate-300 text-sm">Questions in pool: {q.questionPool?.length || 0}</p>
                </div>
      ...
      <Modal isOpen={isAssignOpen} onClose={() => setIsAssignOpen(false)} title={`Assign Quiz: ${selectedQuiz?.title || ""}`}>
        <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto p-2 bg-slate-800 rounded-md">
              <label className="flex items-center gap-2 p-2 hover:bg-slate-700 rounded">
                <input
                  ...
                  onChange={async (e) => {
                    const next = e.target.checked ? students.map(s => s._id) : [];
                    setSelectedStudentIds(next);
                    try { await applyAssignment(next); } catch {}
                  }}
                />
                Select All
              </label>
              {students.map((s) => (
                <label key={s._id} className="flex items-center gap-2 p-2 hover:bg-slate-700 rounded">
                  <input
                    type="checkbox"
                    checked={selectedStudentIds.includes(s._id)}
                    onChange={async (e) => {
                      setSelectedStudentIds((prev) => {
                        const next = e.target.checked ? [...prev, s._id] : prev.filter(id => id !== s._id);
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <label className="block">
              ...
              <input type="checkbox" className="ml-2" checked={isLive} onChange={(e) => setIsLive(e.target.checked)} />
            </label>
            {!isLive && (
              <label className="block">
                <span className="text-gray-300">Deadline</span>
                <input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="mt-1 block w-full p-2 rounded-md bg-slate-700 border-slate-600" />
              </label>
            )}
            <label className="block">
              <span className="text-gray-300">Time limit (mins)</span>
              <input type="number" value={timeLimit} onChange={(e) => setTimeLimit(parseInt(e.target.value) || 0)} className="mt-1 block w-full p-2 rounded-md bg-slate-700 border-slate-600" />
            </label>
          </div>

          <Button onClick={() => setIsAssignOpen(false)} disabled={submitting} className="w-full">
            Close
          </Button>
        </div>
      </Modal>
