...
import { DocumentTextIcon, SparklesIcon, YoutubeIcon } from "../../components/Icons";
    ...
    <>
      <AnimatedWrapper className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold" style={{ color: 'var(--text)' }}>Learning Resources</h2>
          <button
            onClick={() => setIsYoutubeSidebarOpen(!isYoutubeSidebarOpen)}
            className="p-1.5 rounded-lg hover:bg-red-500/10 transition-all duration-200 group"
            title="YouTube Search"
          >
            <YoutubeIcon className="w-9 h-9 text-red-600 transition-transform group-hover:scale-110" />
          </button>
        </div>
        <Card>
